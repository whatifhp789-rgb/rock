...
DEFAULTS = {
    "bot_token": "8845149232:AAFFdh_TrT4eIoImwN47YkndlLXPuMcqbNs",
...
def duplicate_action(chat_id, tag, window=8):
    ...
    Second safety net: even if two copies of the bot (or two Telegram
    deliveries) reach the handler, only the first one answers.
    """
    key = f"{chat_id}:{tag}"
    now = time.time()
    with db() as c:
        ...
        if row and now - row["ts"] < window:
            return True
        c.execute(
            "INSERT INTO seen_actions (key, ts) VALUES (?, ?) "
...
def handle_callback(cq):
    ...
    if data == "howto" or data.startswith("plan:"):
        if duplicate_action(chat_id, "cb:" + data, 5):
            answer()
            return
...
def acquire_lock():
    ...
    Uses a real OS lock (fcntl on Linux/macOS/Termux, msvcrt on Windows) so a
    stale bot.lock file or a recycled pid can never let two copies run.
    """
    global _LOCK_FH
    ...
    try:
        ...
        _LOCK_FH.seek(0)
        other = _LOCK_FH.read().strip()
        sys.exit(
            f"Another bot instance is already running{' (pid ' + other + ')' if other else ''}.\n"
    ...
    def _release():
        try:
            _LOCK_FH.close()
            os.path.exists(LOCK_PATH) and os.remove(LOCK_PATH)
        except OSError:
            pass
    ...
    while True:
        try:
            ...
            if res.get("error_code") == 409:
                # another copy of the bot is polling the same token elsewhere
                print("[conflict] Another instance is polling this bot token. "
                      "Stop it — that is what causes duplicate replies.")
                time.sleep(5)
                continue
